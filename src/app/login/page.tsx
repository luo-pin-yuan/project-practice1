  ...
  login: async (email: string, password: string) => {
    try {
      ...
      const response = await fetch(
        ...
        `https://ottfwogpkzhitdekrnkq.supabase.co/rest/v1/UsersTbl?select=*&email=eq.${encodeURIComponent(
          email
        )}`,
        {
          // 使用 GET 方法
          ...
          headers: {
            // 設定 apikey
            apikey:
              "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im90dGZ3b2dwa3poaXRkZWtybmtxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDkxMzQxMzMsImV4cCI6MjA2NDcxMDEzM30.56I4EssOz4RGnPcKeHl-vLI0D_QYEPbuKdzxWjMYmXU",
          },
        }
      );
...
export default function LoginForm() {
  ...
  const handleLogin = async () => {
    try {
      ...
      const resultUserInfo = await authService.login(
        formData.email,
        formData.password
      );
  ...
  return (
    <div className="flex justify-center items-center h-full bg-gray-100 gap-4">
      ...
      <Card
        title="使用者資訊"
        className={`w-96 ${isInfoVisible ? "" : "hidden"}`}
      >
        <div className="flex flex-col gap-4 ">
          ...
          <div className={`${cssUserInfoItem}`}>
            ...
            <div className="p-2 bg-gray-100 rounded">{userInfo?.password}</div>
          </div>
          <div className={`${cssUserInfoItem}`}>
          ...
          <div className={`${cssUserInfoItem}`}>
            ...
            <div className="p-2 bg-gray-100 rounded">{userInfo?.gender}</div>
          </div>
        </div>
      </Card>
    </div>
